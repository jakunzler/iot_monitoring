...
1. **`setup_server_production.py`** - Script para configurar o servidor na máquina com IP fixo
2. **`setup_module_5g.py`** - Script para atualizar configuração do módulo 5G
3. **`server_production.py`** - Versão otimizada do servidor para produção
4. **`gunicorn.conf.py`** - Configuração do Gunicorn para produção
5. **`monitor_server.py`** - Script de monitoramento do servidor

### Arquivos de Configuração

1. **`.env`** - Variáveis de ambiente para o módulo 5G
2. **`test_server_connection.py`** - Script de teste de conectividade
3. **`deploy_to_picarx.sh`** - Script de deploy no PiCarX
...
```bash
# Na máquina com IP fixo 200.137.220.50
cd /path/to/backend
...
Este script irá:
- Atualizar `PUBLISH_URL` no `publish_picarx_dht22.py`
