...
# === Pino do sensor ===
# Permitir troca do pino via env var (BCM). Padrão: 17 (pino físico 11)
def _resolve_board_pin_from_bcm(env_value: str):
    ...
    return mapping.get(bcm, board.D17)

_bcm_str = os.getenv("DHT_GPIO_BCM", "17")
...
def main():
    ...
    # Cria o sensor aqui (e não em nível de módulo) para capturar erros de init
    dht = None
    while dht is None:
        try:
            dht = create_sensor()
    ...
    while True:
        try:
            ...
            try:
                # Em algumas versões, DHT22 não expõe exit(); ignore se não existir
                if hasattr(dht, "exit"):
                    dht.exit()
            except Exception:
                pass
